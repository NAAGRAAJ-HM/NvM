...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE)  NvM_Crc_Crc16_Calculate(NvM_ConstRamAddressType data_pt, uint16 length, NvM_CrcValuePtrType currCrc)
{
    ...
    *(NvM_Crc16PtrType)currCrc = LibAutosarCrc_u16Calculate(data_pt, length, *(NvM_Crc16PtrType)currCrc, FALSE);
}
...
{
    *currCrc = LibAutosarCrc_u32Calculate(data_pt, length, (*currCrc) ^ NVM_CRC32_XOR_VALUE, FALSE);
}
